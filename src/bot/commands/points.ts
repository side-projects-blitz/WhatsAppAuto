...
import {
  getPointsFromMessage,
  getPreviousPointsFromMessage,
} from "../utils/getPointsFromMessage";
import { getTemplatePoints, setTemplatePoints } from "../data/templatePoints";

const formatter = new Intl.NumberFormat("es-ES");

export const handleAddPoints = async (
...
export const getNewPoints = async (
  chat: WAWebJS.Chat,
  quotedMessage: WAWebJS.Message
) => {
  const chatMessages = await chat.fetchMessages({
    limit: 500,
  });

  const firstMessageIndex = chatMessages.findIndex(
    (msg) => msg.id.id === quotedMessage.id.id
  );

  const messagesWithPoints = chatMessages.slice(firstMessageIndex + 1);

  const allPoints = messagesWithPoints.flatMap((msg) =>
    getPointsFromMessage(msg.body)
  );

  return allPoints.reduce((acc, num) => acc + num, 0);
};

export const handleAddPointsTemplate = (
  message: WAWebJS.Message,
  chat: WAWebJS.Chat
) => {
