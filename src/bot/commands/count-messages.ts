...
export const handleCountMyMessages = async (message: WAWebJS.Message, chat: WAWebJS.Chat) => {
    ...
    if (quotedMessage) {
        messagesCount = myMessages.filter((msg) => msg.timestamp >= quotedMessage.timestamp).length;
    } else {
